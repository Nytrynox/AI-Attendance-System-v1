...
"""
Enhanced Main Window with Automatic Liveness Detection and Anti-Spoofing
Fixed version without syntax errors - integrates all security features
"""
...
class EnhancedLivenessWindow:
    ...
    def start_attendance_mode(self):
        ...
        # New challenge for this session
        self.current_challenge = get_random_challenge()
        self.update_status(f"🧠 Challenge: {self.current_challenge['instruction']}")
    ...
    def process_enhanced_frame(self, frame):
        ...
        try:
            ...
            for face in faces:
                ...
                if not is_live:
                    log_spoof_attempt(reason, frame=frame, user_id=None, context='attendance')
                    cv2.rectangle(frame_copy, (x, y), (x+w, y+h), (0, 0, 255), 8)
                    cv2.putText(frame_copy, f"{reason}", (x, y-10), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
                    self.update_liveness_status_threadsafe(f"❌ {reason}")
                    continue
                
                # If challenge is passed, show success
                ...
                if user_id and user_id in [u[0] for u in self.registered_users] and confidence > 0.7:
                    ...
                    if user_id in self.last_recognition_time:
                        ...
                        if time_since_last < self.recognition_cooldown:
                            # Still in cooldown
                            cv2.rectangle(frame_copy, (x, y), (x+w, y+h), (0, 255, 255), 3)
                    ...
                    if liveness_result == "in_progress":
                        ...
                        cv2.rectangle(frame_copy, (x, y), (x+w, y+h), (255, 165, 0), 4)
                        cv2.putText(frame_copy, "🔍 VERIFYING LIVENESS...", 
                                  (x, y-55), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 165, 0), 2)
                        ...
                        if user_id in self.user_liveness_data:
                            ...
                            progress_width = x+w - x
                            cv2.rectangle(frame_copy, (x, y+h+5), 
                                        (x + int(progress_width * progress), y+h+15), 
                                        (255, 165, 0), -1)
                            ...
                            cv2.putText(frame_copy, f"👁️ Blinks:{blink_status} | 🔄 Movement:{movement_status}", 
                                      (x, y+h+35), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 165, 0), 1)
                        
                        continue
                    
                    elif liveness_result == "failed":
                        # Liveness verification failed
                        cv2.rectangle(frame_copy, (x, y), (x+w, y+h), (0, 140, 255), 4)
                        cv2.putText(frame_copy, "❌ LIVENESS FAILED", 
                                  (x, y-35), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 140, 255), 2)
                        cv2.putText(frame_copy, "👁️ Blink naturally & move head", 
                                  (x, y-10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 140, 255), 2)
                        ...
                        cv2.putText(frame_copy, "✅ ATTENDANCE MARKED!", 
                                  (x, y-80), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 0), 3)
                        cv2.putText(frame_copy, "🔒 Liveness Verified", 
                                  (x, y-55), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 2)
                        ...
                        if pulse:
                            cv2.rectangle(frame_copy, (x-5, y-5), (x+w+5, y+h+5), (0, 200, 0), 3)
                    ...
                    cv2.rectangle(frame_copy, (x, y), (x+w, y+h), (128, 128, 128), 2)
