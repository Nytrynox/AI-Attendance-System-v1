...
class CompleteMainWindow:
    ...
    def open_register_window(self):
        """Open the registration window with advanced liveness detection"""
        try:
            from src.gui.add_user_window_advanced_liveness import AdvancedLivenessAddUserWindow
            window = tk.Toplevel(self.master)
            AdvancedLivenessAddUserWindow(window, on_close_callback=self.refresh_data)
            self.update_status("🔒 Advanced liveness registration window opened.")
        except ImportError as e:
            self.update_status(f"Error: Could not open advanced registration window. {str(e)}")

    def open_register_window_liveness(self):
        """Open the registration window with enhanced liveness detection"""
        try:
            # Try the enhanced liveness detection version first (most secure)
            from src.gui.add_user_window_liveness_enhanced import launch_liveness_enhanced_window
            launch_liveness_enhanced_window(parent=self.master, on_close_callback=self.refresh_data)
            self.update_status("🔒🔵 Enhanced liveness detection registration window opened.")
        except ImportError:
            try:
                # Fallback to enhanced anti-spoofing version
                from src.gui.add_user_window_anti_spoof_enhanced import launch_anti_spoof_enhanced_window
                launch_anti_spoof_enhanced_window(parent=self.master, on_close_callback=self.refresh_data)
                self.update_status("🔒 Enhanced anti-spoofing registration window opened.")
            except ImportError:
                try:
                    # Fallback to enhanced fixed version
                    from src.gui.add_user_window_enhanced_fixed import launch_enhanced_add_user_window
                    launch_enhanced_add_user_window(parent=self.master, on_close_callback=self.refresh_data)
                    self.update_status("Enhanced registration window opened.")
                except ImportError:
                    try:
                        # Fallback to fixed version
                        from src.gui.add_user_window_fixed import FixedAddUserWindow
                        register_window = tk.Toplevel(self.master)
                        FixedAddUserWindow(register_window, on_close_callback=self.refresh_data)
                        self.update_status("Fixed registration window opened.")
                    except ImportError:
                        # Final fallback to original version
                        from src.gui.add_user_window import launch_add_user_window
                        launch_add_user_window()
                        self.master.after(1000, self.refresh_data)
                        self.update_status("Original registration window opened.")
        except Exception as e:
            self.update_status(f"Error opening registration window: {str(e)}")
