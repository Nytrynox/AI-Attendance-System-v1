...
class EnhancedAddUserWindow:
    def __init__(self, master):
        self.master = master
        self.master.title("Add New User - Enhanced Camera")
    ...
    def camera_loop(self):
        ...
        self.update_status("Starting camera - please wait...")
        
        while self.camera_running:
            try:
                # Check if camera is still valid
                if self.cap is None:
                    print("Camera is None, exiting camera loop")
                    self.update_status("Camera error: Camera object is None")
                    break
                
                if not self.cap.isOpened():
                    print("Camera is not open, exiting camera loop")
                    self.update_status("Camera error: Camera is not open")
                    break
                ...
                if not ret or frame is None:
                    error_count += 1
                    print(f"Failed to read frame (error #{error_count})")
                    
                    if error_count >= max_errors:
                        self.update_status("Camera error: Unable to read frames")
                        print("Too many frame reading errors, exiting camera loop")
                        break
                    
                    # Try to re-initialize the camera if we have persistent errors
                    if error_count == 5:
                        print("Multiple read errors, reinitializing camera")
                        self.cap.release()
                        self.cap = self.camera_manager.initialize_camera()
                        if self.cap is None:
                ...
                self.update_status(f"Camera error: {str(e)[:50]}")
                
                # Count errors to prevent infinite error loops
                error_count += 1
                if error_count >= max_errors:
                    print("Too many errors in camera loop, exiting")
                    break
                    
                time.sleep(0.1)  # Brief pause to avoid CPU spinning on errors
    ...
    def capture_face(self):
        ...
        try:
            ...
            if not is_real:
                ...
                return
                  # Get face encoding for recognition with enhanced approach
            face_encoding = None
            error_details = []
            
            try:
                # Method 1: Try FaceRecognizer's robust method first
                try:
                    from src.face_recognizer import FaceRecognizer
                    temp_recognizer = FaceRecognizer()
                    face_encoding = temp_recognizer.extract_face_embedding(face_crop)
                    self.update_status("✅ Face encoding extracted successfully")
                except Exception as e1:
                    error_details.append(f"FaceRecognizer method: {str(e1)[:50]}")
                
                # Method 2: Try preprocess_for_face_recognition as fallback
                if face_encoding is None:
                    try:
                        face_encodings = preprocess_for_face_recognition(face_crop)
                        if face_encodings:
                            face_encoding = face_encodings[0]
                            self.update_status("✅ Face encoding extracted using preprocessing")
                        else:
                            error_details.append("Preprocessing method: No encodings returned")
                    except Exception as e2:
                        error_details.append(f"Preprocessing method: {str(e2)[:50]}")
                
                # Method 3: Direct face_recognition library as last resort
                if face_encoding is None:
                    try:
                        ...
                        if h < 80 or w < 80:
                            ...
                            rgb_face = cv2.resize(rgb_face, (new_w, new_h), interpolation=cv2.INTER_CUBIC)
                        
                        # Try to detect face locations
                        face_locations = face_recognition.face_locations(rgb_face)
                        if not face_locations:
                            # Use whole image as face location
                            h, w = rgb_face.shape[:2]
                            face_locations = [(0, w, h, 0)]
                        
                        # Extract encodings
                        direct_encodings = face_recognition.face_encodings(rgb_face, face_locations)
                        if direct_encodings:
                            face_encoding = direct_encodings[0]
                            self.update_status("✅ Face encoding extracted using direct method")
                        else:
                            error_details.append("Direct method: No encodings extracted")
                    except Exception as e3:
                        error_details.append(f"Direct method: {str(e3)[:50]}")
                
                # Check if we got a face encoding
                if face_encoding is None:
                    # Show detailed error message
                    error_msg = (
                        "❌ FACE FEATURE EXTRACTION FAILED\n\n"
                        "The system could not extract facial features from your image.\n\n"
                        "TROUBLESHOOTING STEPS:\n"
                        "✅ Ensure your face is well-lit (avoid shadows)\n"
                        "✅ Look directly at the camera\n"
                        "✅ Move closer to the camera (2-3 feet away)\n"
                        "✅ Remove glasses, hats, or masks if possible\n"
                        "✅ Ensure camera is clean and in focus\n"
                        "✅ Try better lighting conditions\n"
                        "✅ Position your face to fill 1/3 of the camera view\n\n"
                        f"Image details: {face_crop.shape if face_crop is not None else 'None'}\n"
                        f"Attempts made: {len(error_details)}\n"
                        "All extraction methods failed - please try again with better conditions."
                    )
                    messagebox.showerror("Face Recognition Error", error_msg)
                    self.update_status("❌ Face feature extraction failed - improve conditions and retry")
                    return
                    
            except Exception as e:
                error_msg = (
                    "❌ FACE PROCESSING ERROR\n\n"
                    "An unexpected error occurred while processing your face.\n\n"
                    "RECOVERY STEPS:\n"
                ...
                self.update_status(f"❌ Face processing error: {str(e)}")
                return
...
# Function to launch the enhanced add user window
def launch_enhanced_add_user_window(parent=None, on_close_callback=None):
    """
    Launch the enhanced add user window
    
    Args:
        parent (tk.Tk, optional): Parent window. If None, creates a new window.
        on_close_callback (callable, optional): Function to call when window closes
    """
    if parent:
        # Create as a top-level window
        window = tk.Toplevel(parent)
        window.title("Add New User - Enhanced")
    ...
    app = EnhancedAddUserWindow(window)
    
    # Modify close_window to call the callback
    original_close = app.close_window
    
    def enhanced_close_window():
        original_close()
        if on_close_callback:
            on_close_callback()
    
    app.close_window = enhanced_close_window
    
    # Set the protocol
    window.protocol("WM_DELETE_WINDOW", app.close_window)
    
    if not parent:
        # Only enter mainloop if this is a standalone window
        window.mainloop()
