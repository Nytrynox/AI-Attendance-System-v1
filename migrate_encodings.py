...
This ensures all registered users are properly loaded by the attendance system.
"""
              ...
              if os.path.exists(legacy_file):
                try:
                    ...
                    if os.path.exists(new_file_path):
                        print(f"[INFO] New format file already exists for user {data.get('name', user_id)}, skipping migration.")
                    else:
                        # Save in new format
                        with open(new_file_path, "wb") as f:
                            pickle.dump(data, f)
                        
                        print(f"[INFO] Migrated {data.get('name', user_id)} (ID: {user_id}): face_encoding.pkl -> {new_filename}")
                        migrated_count += 1
