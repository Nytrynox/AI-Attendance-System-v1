...
class DualCameraWindow:
    ...
    def __init__(self, master):
        ...
        # Defensive check to prevent errors if directory doesn't exist
        if os.path.exists(os.path.join('data', 'registered_users')):
            self.registered_users = load_registered_users()
            self.face_recognizer.reload_user_data()
        else:
            self.registered_users = {}
            logger.warning("Registered users directory not found. Skipping user loading.")
        ...
        try:
            ...
            for face in faces:
                ...
                if user_id:
                    ...
                    is_live, verification_complete, status_message = self.liveness_detector.verify_liveness(
